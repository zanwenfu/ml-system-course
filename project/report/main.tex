...
\begin{abstract}
I conduct a controlled, three-family comparison of speculative decoding on a T4 GPU---bandwidth-bound hardware where system-level effects dominate theoretical predictions.
Sequoia's hardware-aware tree optimizer predicts $1.68\times$ speedup for the DP-optimal $(n{=}16, d{=}3)$ tree; I measure $0.56\times$.
...
I test this by porting Sequoia's hardware-aware optimizer~\cite{chen2024sequoia} to a T4 GPU, predicting a $1.68\times$ speedup, and measuring the gap.
I further hypothesize that the gap is not random implementation noise but decomposes into a small number of independently-measurable terms, each traceable to a specific assumption.
...
I argue this assumption fails on bandwidth-bound hardware and propose instead to ground the cost model in \emph{measured wall-clock per-token decode time}.

I define $C_t$ and $C_d$ as the measured wall-clock times for a single-token forward pass of the target and draft models, respectively.
On bandwidth-bound hardware this diverges sharply from parameter-count proxies: my 1B draft and 3B target setup (Llama-3.2 family, fp16, T4 GPU) yields $C_d = 22.42$\,ms and $C_t = 37.63$\,ms, giving a cost ratio $c = C_d/C_t = 0.596$---nearly double the parameter ratio of $0.333$.
...
Speculative decoding was independently introduced by Leviathan et al.~\cite{leviathan2023speculative} and Chen et al.~\cite{chen2023accelerating}; subsequent work extended this into three families I study here---linear draft-verify~\cite{leviathan2023speculative,chen2023accelerating,hf_assisted}, tree-structured speculation (SpecInfer~\cite{miao2024specinfer}, Sequoia~\cite{chen2024sequoia}, and single-model variants Medusa~\cite{cai2024medusa}, Hydra~\cite{ankner2024hydra}, EAGLE~\cite{li2024eagle}), and retrieval-based methods (PLD~\cite{saxena2023pld}, REST~\cite{he2024rest}, SAM-Decoding~\cite{hu2024samdecoding}).
I exclude single-model variants from evaluation because they require fine-tuning outside my scope.
...
My cost model draws on the roofline framework of Williams et al.~\cite{williams2009roofline} but differs in emphasis: LLM decoding at batch 1 is below the roofline knee for all current GPUs, so the relevant axis is not arithmetic intensity but the \emph{absolute} bandwidth tax, which varies $\sim$40$\times$ between T4 ($\sim$20\,ms per 1B fp16 forward) and A100 ($\sim$0.5\,ms).
To my knowledge this is the first controlled cross-family study on bandwidth-bound hardware that quantitatively decomposes the gap against a published cost model and reconciles it to Python-orchestration noise.
...
Beyond the standard HuggingFace \texttt{generate()} API, I implemented eight measurement subsystems that collectively required $\sim$2{,}000 lines of custom code.
...
Unlike HuggingFace's \texttt{num\_assistant\_tokens} interface, which reports only aggregate throughput, my loop instruments the accept/reject decision at each position, providing the data for break-even analysis (\S\ref{sec:linear}).
...
Reimplementation of Algorithm~1 in Appendix~F.1.1 of~\cite{chen2024sequoia} with my measured acceptance vector, exploring tree structures up to size 64 and depth 6.
...
I test this directly with a controlled probe: single-token incremental decoding with a persistent KV cache at $L \in \{30, 64, 128, 256\}$ (Table~\ref{tab:probe}).
...
For the 1B fp16 draft, the weight-loading floor alone is $2\,\text{GB} / 300\,\text{GB/s} \approx 6.7$\,ms; the residual $\sim$13\,ms comprises KV-attention reads (which scale with $L$ but contribute a few hundred microseconds at these lengths), kernel launch overhead, and small elementwise ops.
...
Fourth, as I show next (\S\ref{sec:tree}), it directly causes Sequoia's cost model to overestimate speedup by $3\times$---and an attempted optimization to recover that gap (\S\ref{sec:fourth}) makes things \emph{worse} for the same reason.
...
I feed my measured T4 parameters into this model and test its predictions against my implementation.
...
My minimal Sequoia-compliant runtime implements the DP tree optimizer, tree attention with the cover-property mask, and within-iteration draft KV cache persistence with branch-level fork/consume-in-place semantics.
...
I measure significantly fewer accepted tokens per iteration.
I attribute this primarily to small-sample noise at deeper acceptance positions ($p_4$ and $p_5$ have only 9 and 6 samples respectively in my 305-sample measurement); positional dependence on context likely contributes but I did not measure it directly.
...
I measure $128.1$\,ms for $(16,3)$, comprising a full prefix rebuild ($32.9$\,ms) plus four single-token extends ($4 \times 23.8 = 95.2$\,ms).
The probe (\S\ref{sec:probe}) confirms each extend hits the $\sim$20\,ms bandwidth floor regardless of cache length.
A natural question follows: if prefix-rebuild adds $\sim$33\,ms per iteration, can I eliminate it by persisting the draft cache across iterations? This is the question \S\ref{sec:fourth} answers.
...
I implemented this optimization (v3, infrastructure item~7) and benchmarked it.
...
Each extension costs essentially $C_d + C_t \approx 20 + 35 = 55$\,ms regardless of how few tokens I add; I measure $67.2$\,ms (the residual is Python orchestration of two sequential forwards).
...
Sequoia's published model has three explicit terms (verify, draft, $C_{\text{sys}}$); I measure $C_{\text{sys}} \approx 0.1$\,ms/iter (\S\ref{sec:profiler}), confirming it correctly captures bookkeeping (truncate, fork).
...
I use PyTorch's CUDA profiler to validate two predictions of the bandwidth-floor model: that $C_{\text{sys}}$ is negligible (KV bookkeeping is not a bottleneck), and that an earlier observational claim about ``framework amortization'' was a measurement artifact.
...
My calibrated four-term formula reconciles to within Python-orchestration noise, confirming that the algorithm is sound and only the cost parameters need recalibration.
...
Several factors limit the scope of my claims.
(1)~My tree implementation does not include CUDA graphs or custom kernels, which production frameworks~\cite{vllm_docs,trtllm_docs} use to mitigate per-call fixed overhead; the gap to a graph-fused implementation is bounded above by my $T_f \approx 13$\,ms per call.
...
At larger batch sizes, arithmetic intensity rises and the bandwidth-bound regime weakens; my findings predict that batch-$B$ throughput-optimized serving on T4 would show progressively smaller cost-model gaps as $B$ grows.
(4)~I did not directly cross-validate the v3 ``qualitative inversion'' on A100 hardware; the prediction follows from the bandwidth-floor model rather than from measurement.
(5)~My 10-prompt suite emphasizes generation quality over coverage; a larger benchmark (e.g., MT-Bench, HumanEval) would tighten the per-prompt variance estimates.
...
Run-to-run variation in throughput is $\le 5\%$ across my 30-trial samples; the qualitative findings (linear SD slowdown at $k\geq 5$, tree slowdown for both v2 and v3, PLD speedup) are robust to seed and prompt-suite variation in my preliminary cross-validation.

\paragraph{Generalizable rule.}
I can derive a closed-form prediction for when the four assumptions break.
...
For my T4 + 1B/3B setup: $T_w^d \approx 6.7$\,ms, $T_w^t \approx 20$\,ms, $T_f \approx 13$\,ms (residual from probe), so $c \approx (6.7+13)/(20+13) = 0.60$, matching my measured $c=0.596$.
...
Sequoia's predicted $1.68\times$ vs.\ my measured $0.56\times$ on T4 establishes the cost-model gap; the four-term calibrated formula reconciles to within $1.1\%$ of measurement, establishing that the gap is not noise but a structured decomposition.
...
This was an individual project; all contributions are my own.

\textbf{Research design.}
I defined the research hypotheses, surveyed the speculative decoding literature, and designed the three-family experimental methodology.

\textbf{Implementation.}
I built the full measurement infrastructure described in Section~\ref{sec:infra}---approximately 2,000 lines of custom Python across eight subsystems, including the $\alpha$-instrumented linear SD loop, the standalone $C_d$/$C_t$ and per-call cost probes, the 305-sample positional acceptance measurement, the Sequoia DP optimizer port, and both tree-structured runtimes (v2 within-iteration and v3 cross-iteration KV persistence), each validated against a 20/20 token-identical correctness gate versus the greedy baseline.

\textbf{Analysis and writing.}
I ran all benchmarks, performed the four-term gap decomposition and calibrated-formula reconciliation, identified and corrected the checkpoint-2 framework-amortization artifact using the controlled $C_d$ probe, derived the closed-form bandwidth-floor cost model ($c = (T_w^d + T_f)/(T_w^t + T_f)$), and produced all tables, figures, and writing.
